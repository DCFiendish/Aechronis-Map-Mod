...
public class AechronisMapMod implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
			...
			// LOCAL TEST SERVER: matches localhost/127.0.0.1 (see C:\aetest) instead of
			// aechronis.net or the private fork's IP. Do not merge this into master.
			var serverData = client.getCurrentServer();
			String serverAddress = serverData != null ? serverData.ip : null;
			if (serverAddress == null || !(serverAddress.toLowerCase().contains("localhost")
					|| serverAddress.contains("127.0.0.1"))) {
